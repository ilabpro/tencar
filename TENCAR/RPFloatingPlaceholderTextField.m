...
- (void)setupViewDefaults
{
    ...
    self.floatingLabel.font = [UIFont fontWithName:@"ALSSchlangesans-Bold" size:12.0f];
...
- (void)drawRect:(CGRect)aRect
{
    ...
    if (self.shouldDrawPlaceholder) {
        UIColor *placeholderGray = self.defaultPlaceholderColor ?: [UIColor colorWithRed:124/255.f green:124/255.f blue:124/255.f alpha:0.8f];
        // Inset the placeholder by the same 5px on both sides so that it works in right-to-left languages too
        CGRect placeholderFrame = CGRectMake(0.f, floorf((self.frame.size.height - self.font.lineHeight) / 2.f), self.frame.size.width - 10, self.frame.size.height);
...
- (void)adjustFramesForNewPlaceholder
{
    ...
    self.originalFloatingLabelFrame = CGRectMake(self.originalTextFieldFrame.origin.x + 0.f, self.originalTextFieldFrame.origin.y,
...
{
    return [super editingRectForBounds:UIEdgeInsetsInsetRect(bounds, UIEdgeInsetsMake(0.f, 0.f, 0.f, 25.f))];
}
